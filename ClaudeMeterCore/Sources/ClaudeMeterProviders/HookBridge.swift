...
/// - `StopFailure` — fires when a turn stops because Claude Code failed.
///
...
public enum HookBridge: Sendable {
    ...
    /// The marker filename includes `$$`, the hook process's PID. A versioned JSON
    /// envelope stores the original event and base64 terminal context, including
    /// Herdr's socket, pane ID, and outer terminal cwd. Keeping context in the file
    /// avoids the 255-byte filename limit without changing Claude Code's event.
    ///
    /// `umask 077` runs before any file is created, so the directory and the payload
    /// are owner-only from the start. A hook payload can contain the permission
    /// prompt text, the working directory, and the transcript path, which Claude
    /// Code itself keeps owner-only.
    ...
    /// In order: the filename-route snippet; the pre-umask route snippet; the snippet that spawned
    /// `ps`/`tr` even with no TERM_PROGRAM; the pre-route snippet; the pre-PID fixed
    /// filename.
