...
struct ResetPhraseTests {
    ...
    @Test("Reset countdowns use the reported timestamp regardless of usage")
    func reportedResetCountdown() {
        for used in [0.0, 20, 80, 100] {
            let session = LimitWindow(percentUsed: used, resetsAt: at(2 * 3600 + 14 * 60))
            let weekly = LimitWindow(percentUsed: used, resetsAt: at(3 * 86400 + 8 * 3600))
            #expect(ResetPhrase.spoken(until: session.resetsAt!, asOf: now) == "in 2h 14m")
            #expect(ResetPhrase.spoken(until: weekly.resetsAt!, asOf: now) == "in 3d 8h")
        }
    }
